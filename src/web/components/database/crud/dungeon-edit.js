...
export default class CrudDungeonEdit extends Component{
  constructor(props){
    super(props);
    ...
    axios.get(apiAddress+"dungeons/"+dungeonId)
      .then(response => {
        this.setState({
          ...
          guides: response.data[0].guides,
          ap_easy: response.data[0].attack_power.easy,
          ap_normal: response.data[0].attack_power.normal,
          ap_hard: response.data[0].attack_power.hard,
          weapon: response.data[0].weapon,
    ...
    return(
      this.state.loaded?  
        <div className="App-editor-dungeon-wrapper mx-5">
          <h4>Editing {this.state.name} Data</h4>
          <Form.Row>
            <Form.Group as={Col}>
